...
const AdminProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${process.env.REACT_APP_UPLOAD_API}/api/v1/admin/music/new`, formData, { headers: { "Content-Type": "multipart/form-data" } });
      ...
      <div className="form-container" style={{ marginBottom: "3em" }}>
        <form className="form" autoComplete="off" onSubmit={(e) => handleSubmit(e)}>
          ...
          <p className="form-field">
            <label className="field-text profile-field-text" htmlFor="name">
              Album Name:{" "}
            </label>
          ...
          <div className="form-field" style={{ width: "60%" }}>
            <label className="field-text " style={{ color: "black", marginBottom: ".5em" }} htmlFor="songs">
              Songs:{" "}
            </label>
            <ol className="songlist" ref={songsRef}>
              {[...Array(numOfSongs)].map((index) => {
                return (
                  <li className="song" key={index}>
                    <label className="field-text profile-field-text" htmlFor="song1" style={{ marginTop: "0" }}>
                      Name:
                    </label>
                    <input autoComplete="off" type="text" className="field-input singleSong" name="songs" id="song1" />
                    <label className="field-text profile-field-text" htmlFor="credits">
                      Credit:
                    </label>
              ...
              <div style={{ display: "flex", justifyContent: "space-between" }}>
                <button type="button" className="btn saveSongs text-light bg-secondary" onClick={(e) => addSong(e)}>
                  Add More
                </button>
              </div>
            </ol>
          ...
          <p className="form-field">
            <label className="field-text profile-field-text" htmlFor="category">
              Category:{" "}
            </label>
          ...
          <p className="form-field">
            <label className="field-text profile-field-text" htmlFor="artist">
              Artist:{" "}
            </label>
          ...
          <p className="form-field music-description">
            <label className="field-text profile-field-text " htmlFor="description">
              Description:{" "}
            </label>
            <textarea
              className="music-description-box"
              type="text"
              name="description"
              id="description"
              value={music.description}
              onChange={(e) => setMusic({ ...music, description: e.target.value })}
            />
          </p>
